...
{
    internal class WorkGiver_FightCryptofreeze : WorkGiver_Scanner
    {
        ...
        public override ThingRequest PotentialWorkThingRequest => ThingRequest.ForDef(InternalDefOf.VQE_Cryptofreeze);
        ...
        {
            Cryptofreeze fire = t as Cryptofreeze;
            if (fire == null)
            {
                return false;
            }
        ...
        {
            return JobMaker.MakeJob(InternalDefOf.VQE_BeatCryptofreeze, t);
        }

        public static bool FireIsBeingHandled(Cryptofreeze f, Pawn potentialHandler)
        {
            if (!f.Spawned)
            {
                return false;
